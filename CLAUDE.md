...
- **Contenu d'un `champs_custom JSONB`** : JSON n'a **pas de type date** → dates en **texte ISO 8601** (`"2026-08-07"`), jamais `jj/mm/aaaa` (le format d'affichage = le front). **Jamais d'argent/quantité/poids** dans le JSONB (décimal → `Double`) → vraie colonne `NUMERIC`. La colonne **n'impose aucun type** : c'est `config_champ.type` + validation applicative qui font contrat (§6octies).
